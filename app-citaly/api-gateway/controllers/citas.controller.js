...
const getCitas = async (req, res) => {
  try {
    ...
    // Manejamos el par�metro date (fecha) del frontend
    if (date) {
      whereClause += ' AND c.fecha = ?';
      queryParams.push(date);
    }
    // Si no hay date, usamos los par�metros tradicionales
    else if (fecha) {
      whereClause += ' AND c.fecha = ?';
    ...
    // Manejamos el par�metro status del frontend
    if (status) {
      // Si contiene comas, significa m�ltiples estados
      if (status.includes(',')) {
        const statusList = status.split(',').map(s => s.trim());

        // Para MySQL, necesitamos manejar cada valor individualmente en lugar de usar IN
        // con una lista de par�metros, ya que puede causar problemas
        whereClause += ' AND (';
        ...
        // A�adir cada valor de estado como un par�metro individual
    ...
    // Si no hay status, usamos el par�metro tradicional
    else if (estado) {
      whereClause += ' AND c.estado = ?';
...
const createCita = async (req, res) => {
  try {
    ...
    // Verificar que el servicio est� disponible en la sucursal
...
// GET eventos de calendario (implementaci�n b�sica para evitar error de callback undefined)
const getAppointmentsCalendar = async (req, res) => {
  try {
    // Aqu� puedes implementar la l�gica real seg�n el modelo de tu base de datos
    // Por ahora, se devuelve un array vac�o para evitar errores
...
// GET filtros para citas (implementaci�n b�sica para evitar error de callback undefined)
const getAppointmentsFilters = async (req, res) => {
  try {
    // Aqu� puedes implementar la l�gica real seg�n el modelo de tu base de datos
    // Por ahora, se devuelve un objeto vac�o para evitar errores
    res.json({ success: true, data: {} });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error interno del servidor', error: error.message });
  }
