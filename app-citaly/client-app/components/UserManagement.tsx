          ...
          <div className="flex flex-col md:flex-row gap-2">
            ...
            <Select value={sortBy} onValueChange={setSortBy}>
              ...
              <SelectContent>
                ...
                <SelectItem value="total_appointments-desc">Más Citas</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </CardHeader>
                ...
                <Card key={user.id} className="hover:shadow-lg transition-shadow">
                  <CardContent className="p-4">
                    ...
                    <div className="mt-3 pt-3 border-t text-sm text-gray-500 space-y-1">
                      ...
                      <p>Dirección: {[user.address, user.city, user.state, user.country].filter(Boolean).join(', ')}</p>
                      <p>Contacto de emergencia: {user.emergency_contact_name} {user.emergency_contact_phone ? `(${user.emergency_contact_phone})` : ''}</p>
                      {user.medical_conditions && (
                        <p>Condiciones médicas: {user.medical_conditions}</p>
                      )}
                      <div className="mt-2 pt-2 border-t">
                        <p>Citas totales: {user.total_appointments}</p>
                        <p>Total gastado: {formatPrice(user.total_spent)}</p>
                        <p>Última visita: {user.last_visit ? new Date(user.last_visit).toLocaleDateString() : 'Sin visitas'}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
      ...
      <Dialog open={isDialogOpen} onOpenChange={handleDialogClose}>
        <DialogContent className="lg:max-w-screen-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Complete la información del cliente paso a paso.
            </DialogDescription>
      ...
      <Dialog open={isUserFormOpen} onOpenChange={handleUserFormClose}>
        <DialogContent className="lg:max-w-screen-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Complete la información del usuario y seleccione su tipo.
            </DialogDescription>
          </DialogHeader>
          <UserForm 
